...
class ArticleValidation extends Validation{
    ...
    private function checkTitle($name, $value){
        ...
        if($this->constraint->minLength('titre', $value, 2)){
            return $this->constraint->minLength('titre', $value, 2);
        }
        if($this->constraint->maxLength($name, $value, 12)){
            return $this->constraint->maxLength('titre', $value, 12);
        }
    ...
    private function checkAuthor($name, $value){
        ...
        if($this->constraint->maxLength($name, $value, 5)) {
            return $this->constraint->maxLength('auteur', $value, 5); 
        }
    }
}